...
def history(request):
    status = ''
    form = DataForm()
    return render(request, 'history.html', {'form': form, 'datas': rekap_pengunjung.objects.filter(tanggal= 'null'), 'status': status })  

def export(request):
    status = ''
    form = DataForm()
    return render(request, 'export.html', {'form': form, 'datas': rekap_pengunjung.objects.filter(tanggal = 'null'), 'status': status })  

def exportResults(request, dateTo, dateFrom):
    status = ''
    form = DataForm()
    
    return render(request, 'export.html', {'form': form, 'datas': rekap_pengunjung.objects.filter(tanggal__range = ['{}'.format(dateFrom), '{}'.format(dateTo)]), 'status': status })  
...
def export_range(request):
    ...
    users = rekap_pengunjung.objects.filter(tanggal__range =(['{}'.format(dateFrom), '{}'.format(dateTo)])).values_list('tanggal', 'pukul10', 'pukul11', 'pukul12', 'pukul13', 'pukul14', 'pukul15', 'pukul16', 'pukul17', 'pukul18', 'pukul19', 'pukul20', 'pukul21')
    for user in users:
